...
cpu_monitor.ps1
│
├── Set-ExecutionPolicy          <- linea 1, resuelve permisos
├── Configuracion                <- 3 variables ajustables
├── Inicializacion contadores    <- se ejecuta UNA vez al arrancar
│   ├── CntTotal, CntUsuario, CntSistema, CntInactivo, CntCola
│   ├── Primera lectura (descartada) + Start-Sleep 1s
│   └── CntNucleos (uno por nucleo) + Start-Sleep 1s
│
├── Funciones auxiliares
│   ├── Clear-Pantalla
│   ├── Get-Barra                <- solo ASCII: # y -
│   ├── Write-Uso                <- etiqueta + barra + color
│   └── Get-Frecuencia           <- via CimInstance
│
├── Show-CPU                     <- lee contadores y pinta pantalla
├── Start-Monitor                <- bucle while + Dispose() al salir
└── Punto de entrada             <- equivalente a __main__ de Python
```

> 💡 **¿Por que 2 segundos de espera al inicio y no 1?**
>
> Con 1 segundo algunos equipos lentos o muy cargados al arrancar (especialmente VMs) todavia no tenian datos fiables en los contadores. Con 2 segundos el margen es suficiente en cualquier escenario. Es el mismo principio que cuando un termometro necesita tiempo para estabilizarse antes de dar una lectura fiable.

### Conceptos nuevos de `PowerShell`

1. **Variables con** `$` → Obligatorio siempre. `$nombre`, `$valor`, `$true`, `$false`.
2. `Get-CimInstance` → La puerta de entrada al hardware en Windows. Consulta la base de datos **WMI** donde Windows registra todo sobre el equipo.
3. `-f` **operador de formato** → El equivalente a los `f-strings` de Python:

    ```powershell
    # Ej. en Python
    f"Núcleo {numero:02d}: {uso:.1f}%"
    
    # Ej. en PowerShell
    "Núcleo {0:D2}: {1:N1}%" -f $numero, $uso
    ```

4. `try/finally` **vs** `try/except` → En Python usamos `except KeyboardInterrupt` para capturar el `Ctrl+C`. En PowerShell, `finally` es más fiable para esto porque se ejecuta **siempre**, pase lo que pase.

> 💡 **Analogía de CIM/WMI:** Imagina que Windows lleva un "libro de registro" enorme donde apunta absolutamente todo sobre el hardware y el software: el modelo de cada pieza, cuándo se instaló, qué hace, qué estado tiene. `Get-CimInstance` es el bibliotecario que te ayuda a consultar ese libro. Le dices qué "capítulo" quieres (`Win32_Processor`, `Win32_DiskDrive`...) y él te trae la información.
...
```powershell
# Abrir el PowerShell ISE como administrador y "abrimos script" (📂),
# finalmente ejecutamos con **F5** o "ejecutar script" (▶️).

# En el terminal de PowerShell (administrador), buscamos la carpeta
# donde tenemos el archivo en linea de comando, ejemplo:
...
![image.png](cpu_monitor_ps1/image.png)

---

### En Windows 11 Home

![image1.png](cpu_monitor_ps1/image1.png)
