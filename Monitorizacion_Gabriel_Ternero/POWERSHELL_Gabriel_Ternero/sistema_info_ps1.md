...
│   ├── Get-BytesLegibles
│   ├── Write-Seccion           -> cabecera uniforme para cada bloque
│   └── Write-Dato              -> linea etiqueta/valor alineada con color opcional
│
├── 10 secciones del informe
│   ├── Write-Encabezado        -> fecha, equipo, SO en cabecera
│   ├── Write-SistemaOperativo  -> SO, version, build, uptime, dominio
│   ├── Write-Procesador        -> modelo, nucleos, frecuencias, cache, virt.
│   ├── Write-MemoriaRAM        -> total, usada, modulos fisicos con tipo DDR
│   ├── Write-Almacenamiento    -> discos fisicos + unidades logicas con tabla
│   ├── Write-GPU               -> modelo, memoria, resolucion, driver
│   ├── Write-Red               -> IPv4, IPv6, MAC, gateway, DNS, DHCP
│   ├── Write-BIOS              -> version, fecha, fabricante, placa base
│   ├── Write-Usuarios          -> cuentas locales + sesiones activas
│   └── Write-Resumen           -> tabla compacta con los datos clave
│
├── Show-SistemaInfo            -> llama todas las secciones en orden
└── Punto de entrada            -> SIN bucle while, se ejecuta una sola vez
```

### Novedades destacadas

1. `Win32_PhysicalMemory` **→ los módulos RAM uno a uno**
A diferencia de Python donde `psutil` solo da el total, **WMI** nos permite ver cada módulo **RAM** físico instalado con su capacidad, tipo (DDR4, DDR5...), velocidad en MHz y fabricante. Es información de gran valor para un administrador de sistemas.
2. `Get-LocalUser` **y** `Get-LocalGroupMember`
Cmdlets nativos de PowerShell que listan las cuentas locales del sistema y sus grupos. Marcamos en amarillo los administradores porque es información de seguridad relevante. Si el equipo está en un dominio, estos cmdlets pueden no mostrar las cuentas de dominio, solo las locales.
3. `Join-String` **sustituido por** `-joinJoin-String` es otro cmdlet solo disponible en PowerShell 6+. Lo hemos sustituido por el operador `-join` que es compatible con PowerShell 5.1:

    ```powershell
    # PS 6+ únicamente
    $DNS = $Config.DNSServerSearchOrder | Join-String -Separator ", "
    
    # PS 5.1 compatible
    $DNS = $Config.DNSServerSearchOrder -join ", "
    ```

4. **Detección de tipo DDR via** `SMBIOSMemoryType`
La propiedad `MemoryType` de `Win32_PhysicalMemory` no siempre es fiable en sistemas modernos con DDR5. `SMBIOSMemoryType` es más precisa porque lee directamente la tabla SMBIOS de la placa base.

> 💡 **Analogía del informe estático vs monitores:** Contratar a un nuevo empleado en una empresa tiene dos fases. Primero lees su currículum (sistema_info.ps1): estudios, experiencia, datos fijos. Luego durante el trabajo observas su rendimiento día a día (los monitores): qué produce, cuánto tarda, si tiene problemas. Necesitas ambas cosas para tener una imagen completa.
...
function Write-MemoriaRAM {
    ...
    if ($Pagina) {
        ...
        Write-Dato "Archivo de pagina" ("{0} MB  (en {1})" -f $TotalPag,
    (($Pagina | ForEach-Object {$_.Name}) -join ", "))
    } else {
...
![image.png](sistema_info_ps1/image.png)

![image1.png](sistema_info_ps1/image1.png)

![image2.png](sistema_info_ps1/image2.png)
