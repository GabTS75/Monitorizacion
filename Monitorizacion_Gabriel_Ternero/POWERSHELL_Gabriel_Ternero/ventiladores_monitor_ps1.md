...
│
├── Set-ExecutionPolicy
├── Configuracion              -> 4 umbrales de RPM + referencia para barras
│
├── Funciones auxiliares
│   ├── Get-BarraRPM           -> barra relativa a RPMMaxReferencia (4000)
│   ├── Get-ColorRPM           -> DarkGray/Green/Yellow/Red segun RPM
│   ├── Get-EstadoVentilador   -> texto: Parado/Normal/Alto/Muy alto
│   ├── Write-VentiladorFila   -> fila completa: nombre + RPM + barra + estado
│   └── Get-TipoVentilador     -> deduce CPU/GPU/caja/fuente por nombre
│
├── 4 metodos de lectura (mismo patron que temperatura_monitor.ps1)
│   ├── Get-VentiladoresLHM    -> RPM reales via LibreHardwareMonitor
│   ├── Get-VentiladoresOHM    -> RPM reales via OpenHardwareMonitor
│   ├── Get-VentiladoresWMI    -> Win32_Fan (raramente funciona con RPM)
│   └── Get-DispositivosRefrigeracion -> Win32_CoolingDevice (deteccion sin RPM)
│
├── Show-Ventiladores
│   ├── Estado de fuentes      -> [OK] o [--] para cada metodo
│   ├── Con RPM (LHM/OHM)      -> tabla + resumen + nota GPU + alertas + comparativa
│   ├── Sin RPM (WMI nativo)   -> lista de dispositivos detectados sin datos
│   ├── Sin nada               -> causas + guia
│   └── Guia LHM               -> siempre visible cuando no hay RPM reales
│
├── Start-Monitor
└── Punto de entrada
```
...
1. **La misma arquitectura que temperatura_monitor.ps1**
Esto es intencional. Ambos scripts comparten el mismo patrón de cuatro métodos en cascada porque comparten el mismo problema: Windows no tiene API universal para sensores físicos. Si se logra entender `temperatura_monitor.ps1` también se entenderá este script.
2. **La división** `$ConRPM` **vs** `$SinRPM`
Es la decisión de diseño clave del script. Separamos los resultados en dos categorías porque no es lo mismo detectar un ventilador con sus RPM reales (LHM/OHM) que simplemente saber que existe un dispositivo de refrigeración (WMI nativo). Mostrarlos mezclados confundiría al usuario.
3. **La referencia dinámica en la comparativa**
En el gráfico comparativo usamos `$RPMMaxReal` como referencia en lugar del valor fijo `$RPMMaxReferencia`. Así el ventilador más rápido siempre ocupa la barra completa y las diferencias entre ventiladores se ven claramente, aunque todos vayan despacio.

> **LHM como puente:** Windows y el hardware hablan idiomas diferentes. Windows habla "WMI estándar" y el hardware habla "registros del chip sensor" (NCT6775, IT8686, etc.). `LibreHardwareMonitor` actúa como intérprete: habla directamente con el chip sensor en su idioma nativo, entiende lo que dice, y luego lo traduce a **WMI** para que PowerShell lo pueda leer. Sin ese intérprete, Windows y el sensor no se entienden.
>
> LHM necesita que actives explícitamente una opción para exponer sus datos vía **WMI**. Por defecto LHM muestra los datos en su propia ventana pero **no los comparte** con el sistema **WMI** hasta que se lo indicas.

👉🏻 [SOLUCIÓN para activar el servidor WMI en LHM](ventiladores_monitor_ps1/SOLUCION_WMI_en_LHM.md) 👈🏻

> 💡 **Analogía de los dos modos de LHM:** Es como un cocinero experto que trabaja en su cocina privada (modo normal). Puede preparar platos excelentes y verlos él mismo, pero si no abre una ventanilla al público (Remote Web Server), nadie más puede pedir ni ver lo que cocina. Activar el servidor **WMI** es exactamente abrir esa ventanilla para que PowerShell pueda hacer pedidos.
...
![image.png](ventiladores_monitor_ps1/image.png)

---

>💡 No aparece la sección “Fans” o “Fan” en la ventana del LHM. Lo cual indica que “Es posible que el hardware no tenga sensores de RPM accesibles o que LHM no tenga sensores para el chip sensor de la placa”. 🤷🏻‍♂️
