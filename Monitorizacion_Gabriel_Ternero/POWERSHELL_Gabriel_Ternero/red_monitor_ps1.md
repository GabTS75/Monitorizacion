...
│   └── Por adaptador detectado  -> BytesRx, BytesTx, PktRx, PktTx,
│                                   ErrRx, ErrTx, Descartes
│
├── Funciones auxiliares
│   ├── Get-BytesLegibles
│   ├── Get-BarraRed             -> barra relativa a 1 Gbps (125 MB/s)
│   ├── Get-ColorVelocidad       -> verde/amarillo/rojo segun MB/s
│   ├── Get-InfoAdaptadores      -> IPs, MAC, gateway via Win32_NetworkAdapterConfiguration
│   ├── Get-EstadoAdaptadores    -> conectado/desconectado via Win32_NetworkAdapter
│   └── Get-ResumenConexiones    -> estados TCP via Get-NetTCPConnection
│
├── Show-Red
│   ├── Seccion 1 -> Por adaptador: IP, MAC, bajada, subida, errores
│   ├── Seccion 2 -> Trafico acumulado desde el arranque
│   ├── Seccion 3 -> Conexiones TCP agrupadas por estado
│   ├── Seccion 4 -> Alertas si hay errores elevados
...
1. `$env:COMPUTERNAME`
En PowerShell las variables de entorno del sistema se acceden con `$env:NOMBRE`. Es como `os.environ['NOMBRE']` en Python pero más elegante. `COMPUTERNAME` es el nombre del equipo en la red.
2. `Group-Object`
Agrupa una lista de objetos por una propiedad, exactamente como `GROUP BY` en SQL o `groupby` en Python. Lo usamos para agrupar las conexiones TCP por estado y obtener un resumen compacto en lugar de listar cientos de conexiones individuales.
3. `switch` **en lugar de if/elseif encadenados**
PowerShell tiene el operador `switch` que es mucho más limpio que una cadena de `if/elseif` cuando hay muchos casos posibles. Lo usamos para asignar colores a los estados TCP.
4. **Búsqueda de IPs por similitud**
Los nombres de los adaptadores en los contadores de rendimiento y en WMI no siempre coinciden exactamente. Por eso hacemos una búsqueda parcial comparando las primeras palabras del nombre, lo que funciona en la gran mayoría de los casos.

> 💡 **Analogía de ESTABLISHED vs LISTEN:** Un puerto en estado `LISTEN` es como un portero en la puerta de una discoteca esperando clientes. Un puerto en estado `ESTABLISHED` es una mesa ya ocupada con clientes sentados y comunicándose. `TIME_WAIT` es una mesa que acaba de vaciarse pero el camarero todavía no ha recogido los vasos, esperando por si acaso algún cliente olvidó algo.
...
![image.png](red_monitor_ps1/image.png)

---

### En Windows Home

![image1.png](red_monitor_ps1/image1.png)
