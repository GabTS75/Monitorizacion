...
│
├── Set-ExecutionPolicy
├── Configuracion            -> umbrales separados: RAM (85%) y Pagina (50%)
│
├── Inicializacion contadores
│   ├── CntDisponible        -> bytes de RAM disponibles
│   ├── CntPaginas           -> paginas movidas RAM<->disco por segundo
│   ├── CntFallosPag         -> fallos de pagina por segundo
│   └── CntUsoPagina         -> % uso del archivo de pagina
│
├── Funciones auxiliares
│   ├── Get-BytesLegibles    -> 8589934592 -> "8.00 GB"  <- Nueva
│   ├── Get-Barra
│   ├── Write-Uso
│   ├── Write-Detalle        -> linea etiqueta/valor alineada <- Nueva
│   └── Write-Diagnostico    -> semaforo verde/amarillo/rojo  <- Nueva
│
├── Show-Memoria
│   ├── Seccion 1 -> Uso de RAM con barra y alerta
│   ├── Seccion 2 -> Desglose: total, en uso, disponible, libre
│   ├── Seccion 3 -> Archivo de pagina (Swap de Windows)
│   ├── Seccion 4 -> Actividad de paginacion en tiempo real
│   └── Seccion 5 -> Diagnostico global
│
├── Start-Monitor            -> bucle + Dispose() al salir
└── Punto de entrada
```

## Novedades respecto a cpu_monitor.ps1

1. **Constantes de tamaño integradas en PowerShell**
Una ventaja que no tiene Python sin `psutil`: PowerShell incluye `1KB`, `1MB`, `1GB` y `1TB` como constantes listas para usar, sin necesidad de calcular potencias de 1024.
2. **`Win32_PageFileUsage` → el Swap de Windows**
En lugar de una partición swap como en Linux, Windows usa un archivo llamado `pagefile.sys`. Esta clase WMI nos da su tamaño y uso actual. Puede haber varios en distintas unidades, por eso sumamos todos con `foreach`.
3. **Contadores de paginación**`Pages/sec` y `Page Faults/sec` son métricas que no tienen equivalente directo en el script Python porque `psutil` no las expone. Son especialmente útiles para detectar si el sistema está sufriendo por falta de RAM aunque el porcentaje de uso no sea crítico todavía.

> 💡 **Analogía de Pages/sec:** Imagina que estás cocinando y la encimera (**RAM**) se llena. Cada vez que necesitas un ingrediente que no cabe en la encimera, tienes que ir al armario (disco) a buscarlo y traerlo. `Pages/sec` cuenta cuántas veces por segundo haces ese viaje. Si el valor es alto de forma sostenida, el cocinero pasa más tiempo corriendo al armario que cocinando.
...
![image.png](memoria_monitor_ps1/image.png)

---

### En Windows 11 Home

![image1.png](memoria_monitor_ps1/image1.png)
