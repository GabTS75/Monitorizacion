...
**Windows es el sistema más restrictivo para leer sensores térmicos**. Vamos a gestionarlo con transparencia, intentaremos todo lo que está disponible y explicamos exactamente por qué cada método funciona o no, así que vamos a ello 👇
...
│
├── Set-ExecutionPolicy
├── Configuracion                  -> umbrales separados por tipo: CPU, disco, general
│
├── Funciones auxiliares
│   ├── Get-Barra
│   ├── Write-Temperatura          -> barra en escala 0-120 grados con color
│   ├── Convert-DecikelvinACelsius -> formula: (dK/10) - 273.15
│   └── Test-TemperaturaValida     -> filtra valores WMI erroneos (0, negativos...)
│
├── 4 metodos de lectura (en orden de precision)
│   ├── Get-TemperaturaLHM         -> LibreHardwareMonitor via root\LibreHardwareMonitor
│   ├── Get-TemperaturaOHM         -> OpenHardwareMonitor  via root\OpenHardwareMonitor
│   ├── Get-TemperaturaACPI        -> Zonas ACPI           via root\wmi
│   └── Get-TemperaturaProbe       -> Win32_TemperatureProbe via root\cimv2
│
├── Show-Temperaturas
│   ├── Estado de cada fuente      -> [OK] o [--] con conteo de sensores
│   ├── Con datos                  -> tabla de sensores + alertas + resumen
│   └── Sin datos                  -> causas + guia de instalacion de LHM
│
├── Start-Monitor
└── Punto de entrada
```
...
1. **Cuatro métodos en cascada**
El script lo intenta por todos los medios disponibles, de más a menos preciso. Si LibreHardwareMonitor está corriendo, obtenemos datos de todos los sensores. Si no, intentamos ACPI. Si nada funciona, explicamos exactamente por qué y qué hacer.
2. **El namespace** `root\wmi`
La mayoría de comandos **WMI** usan el namespace `root\cimv2` por defecto. Pero `MSAcpi_ThermalZoneTemperature` vive en `root\wmi`, un namespace diferente. Sin especificarlo explícitamente el comando falla silenciosamente. Es uno de esos detalles que solo se aprenden con experiencia.
3. **La fórmula de decikelvin**
**WMI** almacena las temperaturas ACPI en **decikelvin** (décimas de Kelvin), no en Celsius. La conversión es:

    ```powershell
    Celsius = (DeciKelvin / 10) - 273.15
    Ejemplo: 3232 dK -> (3232/10) - 273.15 = 49.85 °C
    ```

4. `Test-TemperaturaValida`
**WMI** a veces devuelve `0`, valores negativos o `3273` (que sería 54°C pero viene de un sensor desconectado). Filtrar estos valores evita mostrar datos sin sentido en pantalla.

> 💡 **Analogía de los 4 métodos:** Es como intentar saber la temperatura de una habitación. Primero miras el termómetro de pared (LHM, el más preciso). Si no hay, miras el del pasillo (OHM). Si tampoco, abres la ventana y calculas por la sensación (ACPI, aproximado). Y si nada funciona, le preguntas al vecino si tiene frío (WMI Probe, raramente útil). El resultado puede variar mucho según el método, pero siempre intentamos dar algo útil.
...
![image.png](temperatura_monitor_ps1/image.png)

---

### En Windows Home

![image1.png](temperatura_monitor_ps1/image1.png)
