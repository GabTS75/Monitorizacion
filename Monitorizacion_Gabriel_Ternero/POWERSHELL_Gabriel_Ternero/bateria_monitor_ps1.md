...
│
├── Set-ExecutionPolicy
├── Configuracion           -> 4 umbrales: critico(10%), bajo(20%), alto(95%)
│
├── Funciones auxiliares
│   ├── Get-Barra
│   ├── Write-BarraBateria  -> 4 colores: rojo/amarillo/verde/cyan
│   ├── Get-TextoEstado     -> traduce el codigo numerico de WMI a texto
│   ├── Get-TiempoLegible   -> convierte minutos a "Xh YYm", gestiona valor especial
│   ├── Get-SaludBateria    -> calcula % de salud con capacidad diseno vs actual
│   └── Get-EstaEnchufada   -> deduce si hay CA a partir del codigo de estado
│
├── Show-Bateria
│   ├── Sin bateria         -> mensaje diferenciado con causa probable
│   ├── Seccion 1           -> nivel de carga + estado + adaptador + alertas
│   ├── Seccion 2           -> tiempo estimado restante o estado de carga
│   ├── Seccion 3           -> salud: capacidad diseno vs actual, o guia powercfg
│   ├── Seccion 4           -> consejos contextuales segun situacion actual
│   └── Seccion 5           -> diagrama ASCII visual de la bateria
│
├── Start-Monitor
└── Punto de entrada
```

### Lo más destacado de este script

**1. Códigos numéricos de** `BatteryStatus`
A diferencia de Python donde `psutil` nos daba texto directo, WMI devuelve un número del 1 al 11. La función `Get-TextoEstado` actúa como "diccionario de traducción" usando `switch`, que es más limpio que una cadena de `if/elseif`.

**2. El valor mágico** `71582788`
Este número aparece en `EstimatedRunTime` cuando Windows no puede calcular el tiempo restante. Es un valor heredado de la especificación ACPI original. Si no lo filtramos mostraríamos "1193046h 08m" en pantalla, lo que confundiría a cualquier alumno.

**3.** `powercfg /batteryreport`
Es la joya oculta de Windows para la batería. Genera un informe HTML completo con el historial de ciclos de carga y la comparativa de capacidad de diseño vs actual. Lo mencionamos en el script porque WMI no siempre reporta los datos de capacidad, pero `powercfg` sí los tiene siempre. Es un comando que todo administrador de sistemas debería conocer.

> 💡 **Analogía del valor 71582788:** Es como cuando le preguntas a alguien cuánto tiempo tardará en terminar una tarea y responde "no lo sé". Windows no dice "no lo sé" en texto, sino que devuelve ese número especial que significa exactamente eso. Quien diseñó **WMI** eligió ese número tan raro precisamente para que fuera inconfundible y no se confundiera con un tiempo real.
...
![image.png](bateria_monitor_ps1/image.png)

---

### En Windows Home

![image1.png](bateria_monitor_ps1/image1.png)
