...
│
├── Set-ExecutionPolicy
├── Configuracion           -> dos umbrales: alerta (85%) y critico (95%)
│
├── Inicializacion contadores
│   ├── Globales (_Total)   -> lectura, escritura, IOPS, tiempo, cola
│   └── Por disco fisico    -> detecta cada disco y crea sus contadores
│
├── Funciones auxiliares
│   ├── Get-BytesLegibles
│   ├── Get-Barra
│   ├── Write-UsoDisco      -> 4 niveles de color incluyendo Magenta critico
│   └── Write-BarraActividad-> barra relativa a 500 MB/s de referencia
│
├── Show-Almacenamiento
│   ├── Seccion 1 -> Espacio por unidad logica (C:\, D:\...)
│   ├── Seccion 2 -> Velocidad I/O global + ocupacion + cola
│   ├── Seccion 3 -> Velocidad por disco fisico individual
│   └── Seccion 4 -> Informacion de discos fisicos (modelo, interfaz...)
│
├── Start-Monitor           -> bucle + Dispose() de todos los contadores
└── Punto de entrada
```

### Novedades de este script

1. **Disco lógico vs disco físico**
Este script trabaja con ambos niveles al mismo tiempo. `Win32_LogicalDisk` nos da las letras de unidad con su espacio (`C:\`, `D:\`...), mientras que `Win32_DiskDrive` nos da los discos físicos reales con su modelo e interfaz. Los contadores de rendimiento trabajan siempre con discos físicos.
2. **Contadores por disco individual**
La detección automática de discos físicos y la creación de un contador por cada uno es la parte más compleja del script. El sistema de nombres de las instancias en Windows usa el formato `"0 C:"`, `"1 D:"` donde el número es el índice físico y las letras son sus particiones.
3. **Cola del disco**
Igual que la CPU tiene su cola de hilos esperando, el disco tiene su propia cola de operaciones pendientes. Un valor sostenido por encima de 2 es la señal clásica de que el disco es el cuello de botella del sistema.

> 💡 **Analogía del % Disk Time:** Imagina un cajero de supermercado. Si está ocupado el 20% del tiempo, va sobrado. Si está ocupado el 95% del tiempo, hay cola y todo se ralentiza. `% Disk Time` mide exactamente eso, es decir, ¿qué porcentaje del tiempo el disco está atendiendo peticiones?. Cuando se acerca al 100% de forma sostenida, el disco no da abasto.
...
![image.png](almacenamiento_monitor_ps1/image.png)

---

### En Windows Home

![image1.png](almacenamiento_monitor_ps1/image1.png)
