...
│
├── Set-ExecutionPolicy
├── $CarpetaBase             -> ruta automatica via $PSScriptRoot
│
├── Carga de modulos
│   └── foreach con dot-sourcing (. .\script.ps1) + $ModulosOK hashtable
│
├── $Opciones                -> array de hashtables con toda la config del menu
│
├── Funciones de interfaz
│   ├── Get-Indicador        -> [OK] verde o [--] rojo segun disponibilidad
│   ├── Write-Banner         -> cabecera con fecha, equipo, CPU% y RAM% actuales
│   ├── Show-Menu            -> pinta el menu completo con marcos ASCII
│   ├── Show-EstadoModulos   -> pantalla de diagnostico de archivos presentes
│   ├── Show-Transicion      -> pantalla previa a lanzar cada monitor
│   └── Show-Despedida       -> pantalla de cierre
│
├── Invoke-Opcion            -> localiza, valida y ejecuta la opcion elegida
│   ├── Opciones 1-10        -> busca en $Opciones y llama a $Op.Funcion
│   ├── Opcion A             -> Show-EstadoModulos
│   └── Opcion S             -> return $false (salir del bucle)
│
├── Start-Menu               -> bucle while + Read-Host + Invoke-Opcion
├── Test-VersionPS           -> comprueba PS >= 5.1
└── Punto de entrada         -> Test-VersionPS y Start-Menu
```

### Diferencias clave respecto al menu.py de Python

1. **Dot-sourcing en lugar de** `import`

    | Python | PowerShell |
    | --- | --- |
    | `import cpu_monitor` | `. .\cpu_monitor.ps1` |
    | Solo carga funciones | Solo carga funciones |
    | `if __name__ == "__main__"` | `if ($MyInvocation... -ne '.')` |

2. `$PSScriptRoot` **— ruta automática**
En Python usamos rutas relativas directamente. En PowerShell el directorio de trabajo puede no ser el mismo que donde está el script. `$PSScriptRoot` siempre contiene la ruta de la carpeta del script en ejecución, sin importar desde dónde lo lances.
3. `& { bloque }` **— invocar bloques de script**
En Python las funciones se llaman directamente: `funcion()`. En PowerShell cuando una función está guardada en una variable (como `$Op.Funcion`) necesitamos el operador `&` para invocarla: `& $Op.Funcion`.
4. `Read-Host` **en lugar de** `input()`
Es el equivalente exacto del `input()` de Python. Muestra un mensaje y espera que el usuario escriba algo y pulse Enter.

> 💡 **Analogía del dot-sourcing:** Imagina que contratas a varios especialistas (los scripts). El `dot-sourcing` es firmar el contrato con cada uno: están disponibles y listos, pero no empiezan a trabajar hasta que les das una orden concreta. Sin el contrato (sin el punto delante) el especialista entra, hace su trabajo completo y se va. Con el contrato solo espera instrucciones.
