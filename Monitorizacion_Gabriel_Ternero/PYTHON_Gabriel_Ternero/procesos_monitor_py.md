...
│
├── Importaciones
├── Configuración            → TOP_N, umbrales de CPU y RAM por proceso
│
├── Funciones auxiliares
│   ├── limpiar_pantalla(), bytes_a_legible(), barra_progreso()
│   ├── color_cpu()          → Verde/Amarillo/Rojo según % de CPU del proceso
│   ├── color_ram()          → Verde/Amarillo/Rojo según MB de RAM del proceso
│   └── truncar()            → Corta nombres largos para que quepan en la tabla
│
├── recopilar_procesos()     → Itera todos los procesos con process_iter() ← Clave
│
├── mostrar_procesos()
│   ├── Sección 1 — Resumen global    → total, conteo por estado, zombies
│   ├── Sección 2 — Top N por CPU     → tabla con PID, CPU, RAM, estado, hilos
│   ├── Sección 3 — Top N por RAM     → tabla con PID, RAM, %, tiempo activo
│   ├── Sección 4 — Contexto sistema  → CPU global, RAM global, load average
│   └── Sección 5 — Por usuario       → cuántos procesos tiene cada usuario
│
├── iniciar_monitor()  → Con calibración previa de CPU ← Importante
└── if __name__ == ...
```

## Las novedades clave de este script

1. `process_iter()` **con atributos específicos**
En lugar de pedir todos los datos de todos los procesos (muy lento), le decimos exactamente qué queremos leer. Es como ir al supermercado con una lista en lugar de llenarte el carrito de todo.
2. **La calibración previa de CPU**
Este es un detalle que sorprende a muchos estudiantes. `cpu_percent()` necesita **dos mediciones** para calcular el uso, igual que la velocidad del disco o la red. Si la llamamos por primera vez sin haber "arrancado el contador", devuelve `0.0%` para todos. La solución es hacer una pasada inicial silenciosa antes de empezar a mostrar datos.
3. **Triple gestión de errores en el bucle**
...
Estos tres errores son **completamente normales** y esperados. Los procesos nacen y mueren constantemente mientras los lees. No son bugs: son el comportamiento natural de un sistema vivo.

> 💡 **Analogía del zombie:** Un proceso zombie es como ese empleado que ya mandó su carta de dimisión, vació su mesa y se fue a casa... pero Recursos Humanos todavía no lo ha dado de baja en el sistema. No ocupa silla, no consume café, pero sigue "existiendo" en la nómina. Si hay muchos zombies es señal de que algún proceso padre tiene un problema de gestión.
...
![image.png](procesos_monitor_py/image.png)

---

### En Linux

![image1.png](procesos_monitor_py/image1.png)
![image2.png](procesos_monitor_py/image2.png)
