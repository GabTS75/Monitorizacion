...
bateria_monitor.py
│
├── Importaciones             → Incluye 'platform' (ya visto en temperatura)
├── Configuración             → 4 umbrales: crítico(10%), bajo(20%), alto(95%)
│
├── Funciones auxiliares
│   ├── limpiar_pantalla()
│   ├── barra_bateria()       → Barra con 5 colores: rojo/amarillo/verde/cian ← Nueva
│   ├── segundos_a_tiempo()   → Maneja constantes especiales de psutil ← Nueva
│   └── icono_estado()        → Icono + texto según enchufado + nivel ← Nueva
│
├── mostrar_bateria()
│   ├── Sin batería           → Mensaje diferenciado Windows/Linux/VM
│   ├── Sección 1             → Nivel, estado, adaptador enchufado
│   ├── Sección 2             → Tiempo estimado restante o para carga completa
│   ├── Sección 3             → Consejos contextuales de salud
│   ├── Sección 4 (Linux)     → Salud real desde /sys: capacidad diseño vs actual
│   └── Sección 5             → Diagrama ASCII visual de la batería
│
├── iniciar_monitor()
└── if __name__ == ...
```
...
1. **Constantes especiales de psutil**`secsleft` no siempre es un número de segundos normal.

    Puede ser:

    | Valor | Significado |
    | --- | --- |
    | `psutil.POWER_TIME_UNKNOWN` (-2) | El sistema no puede estimar el tiempo todavía |
    | `psutil.POWER_TIME_UNLIMITED` | Enchufado y carga completa, no hay límite |
    | Número positivo | Segundos reales estimados |

2. **Lectura de** `/sys/class/power_supply/` **en Linux**
Esta es la joya oculta del script. Mientras psutil solo nos da tres datos básicos, el kernel de Linux expone **mucha más información** sobre la batería en archivos de texto planos dentro de `/sys`. Leyendo `energy_full_design` y `energy_full` podemos calcular la **salud real** de la batería: cuánta capacidad le queda respecto a cuando era nueva.
3. **Consejos contextuales**
Los consejos solo aparecen cuando son relevantes. Si todo está bien, simplemente dice "estado normal". Este principio, mostrar información solo cuando aporta valor, es una buena práctica de diseño de herramientas de monitorización.

> 💡 **Analogía de la salud de la batería:** Una batería nueva es como un vaso recién comprado que cabe exactamente 500 ml. Con los años y los ciclos de carga, el vaso se va "encogiendo" por dentro aunque por fuera parezca igual. Si ahora solo caben 350 ml, la salud es del 70%. El porcentaje de carga que te muestra el sistema operativo siempre es relativo a esa capacidad actual, no a la original.
...
![image.png](bateria_monitor_py/image.png)

Sin conectar al cargador

![image1.png](bateria_monitor_py/image1.png)
...
![image2.png](bateria_monitor_py/image2.png)

Sin conectar al cargador

![image3.png](bateria_monitor_py/image3.png)
