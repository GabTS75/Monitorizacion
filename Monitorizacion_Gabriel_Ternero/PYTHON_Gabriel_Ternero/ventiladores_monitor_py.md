...
Los ventiladores son, junto a las temperaturas, el script más dependiente del hardware físico. Comparten incluso la misma **API** de `psutil`, así que veremos cierta similitud con `temperatura_monitor.py`.
...
│
├── Importaciones
├── Configuración                → 4 umbrales: mínimo activo, alto, muy alto, referencia
│
├── Funciones auxiliares
│   ├── limpiar_pantalla()
│   ├── barra_rpm()              → Barra en RPM con rpm_max configurable ← Nueva
│   ├── color_rpm()              → Gris/Verde/Amarillo/Rojo + formato RPM ← Nueva
│   ├── estado_ventilador()      → Texto + icono de semáforo ← Nueva
│   └── identificar_ventilador() → Deduce tipo (CPU/GPU/caja...) por nombre ← Nueva
│
├── mostrar_ventiladores()
│   ├── Bloque Windows           → Mensaje + alternativas (SpeedFan, Fan Control...)
│   ├── Sin sensores             → Causas probables y comandos de diagnóstico
│   ├── Sección 1                → Todos los fans por chip con RPM, barra y estado
│   ├── Sección 2                → Alertas activas si hay RPM fuera de rango
│   ├── Sección 3                → Gráfico comparativo ordenado de mayor a menor
...
## El detalle más importante: el modo 0 RPM

Este script incluye una lógica que muchos estudiantes agradecerán entender. Cuando aparece un ventilador a **0 RPM**, el script distingue dos casos completamente diferentes:

| Situación | Significado | Mensaje |
| --- | --- | --- |
| Ventilador GPU a 0 RPM | **Normal** — modo silencioso | *"Las GPUs modernas paran en reposo"* |
| Ventilador CPU/caja a 0 RPM | **Sospechoso** — puede ser un problema | *"Verifica que funcione"* |

> 💡 **Analogía del modo 0 RPM:** Es como el sistema de arranque/parada automático de los coches modernos. Cuando el coche está parado en un semáforo, el motor se apaga solo para ahorrar combustible y reducir ruido. Cuando vuelves a acelerar, arranca en milisegundos. La GPU hace exactamente lo mismo con sus ventiladores: los apaga cuando no los necesita y los arranca cuando la temperatura sube. No es un fallo, es una función.
...
#
#  🖥️  VM      → Las máquinas virtuales no tienen acceso a ventiladores
...
![image.png](ventiladores_monitor_py/image.png)

---

### En Linux

![image1.png](ventiladores_monitor_py/image1.png)
