...
Este script es el más importante, digamos que el “principal”, porque es el que lista un menú de opciones a elegir (estilo panel de control) para ejecutar cada uno de los scripts de monitorización y de información del hardware de un ordenador.
...
│
├── Importación de los 10 scripts
│   └── Cada uno en su propio try/except → si falta uno, el menú sigue vivo
│
├── OPCIONES[]               → Lista de diccionarios con toda la config del menú
├── OPCIONES_ESPECIALES[]    → Opciones A (estado) y S (salir)
│
├── Funciones de interfaz
│   ├── limpiar_pantalla()
│   ├── indicador_disponible()    → ✅ o ❌ según si el .py está presente
│   ├── mostrar_banner()          → Cabecera con fecha, hostname y CPU/RAM actuales
│   ├── mostrar_menu()            → Construye y pinta el menú completo
│   ├── mostrar_estado_modulos()  → Pantalla de diagnóstico de archivos presentes
│   ├── pantalla_lanzando()       → Transición antes de entrar en un monitor
│   └── despedida()               → Pantalla de cierre al salir
│
├── ejecutar_opcion()       → Localiza, valida y lanza la opción elegida
├── iniciar_menu()          → Bucle principal: mostrar → pedir → ejecutar
├── verificar_psutil()      → Comprueba la dependencia antes de arrancar
└── if __name__ == ...      → Verifica psutil y arranca el menú
```

### Los conceptos nuevos de este script

1. **Importar módulos propios con** `import`
Aquí está el concepto más importante de todo el proyecto. Cuando escribes `import cpu_monitor`, Python busca el archivo `cpu_monitor.py` en la misma carpeta y lo carga en memoria. A partir de ese momento puedes usar `cpu_monitor.iniciar_monitor()` como si fuera una función de una librería.
2. **Lista de diccionarios como configuración**
En lugar de un `if/elif` enorme, todas las opciones del menú viven en una lista `OPCIONES`. Cada opción es un diccionario con su número, icono, título, descripción y la función que hay que llamar. Si mañana quieres añadir un nuevo monitor, solo añades un diccionario más a la lista. El resto del código no cambia.
3. `lambda` **— funciones en una línea**
Dentro de cada opción hay una clave `'funcion'` que contiene un `lambda`. Un lambda es una función pequeña sin nombre que se define en una sola línea. `lambda: cpu_monitor.iniciar_monitor()` significa exactamente: *"una función que cuando la llames, ejecute `cpu_monitor.iniciar_monitor()`"*.
4. `sys.exit(1)` **— salir con código de error**
Cuando un programa termina en Linux/Windows, devuelve un número al sistema operativo. `0` significa "todo fue bien". Cualquier otro número significa "algo salió mal". Es una convención universal que usan todos los programas, scripts y servicios.

> 💡 **Analogía del menú como caja de herramientas:** Cada script es como una herramienta especializada. El menú es la caja que las organiza. Las herramientas no se activan solas por estar en la caja — tú decides cuándo sacar cada una. Y si una herramienta falta en la caja (archivo no encontrado), la caja sigue siendo útil con las que sí están.

---

### El Menú de Monitorización

```python
📁 monitoreo/
   ├── menu.py                     ← Panel de control principal
   ├── cpu_monitor.py              ← ✅ Probado en Windows y Linux
   ├── memoria_monitor.py          ← ✅ Probado en Windows y Linux
   ├── almacenamiento_monitor.py   ← ✅ Probado en Windows y Linux
   ├── temperatura_monitor.py      ← ✅ (datos solo en físico Linux/Mac)
   ├── gpu_monitor.py              ← ✅ (datos solo con NVIDIA/AMD real)
   ├── red_monitor.py              ← ✅ Probado en Windows y Linux
   ├── bateria_monitor.py          ← ✅ Probado en Windows y Linux
   ├── ventiladores_monitor.py     ← ✅ (datos solo en físico Linux/Mac)
   ├── procesos_monitor.py         ← ✅ Probado en Windows y Linux
   └── sistema_info.py             ← ✅ Probado en Windows y Linux
```

### Para ejecutarlo

```python
# Requisito indispensable y previio de instalación
pip install psutil
...
![image.png](MENU_PRINCIPAL/image.png)
