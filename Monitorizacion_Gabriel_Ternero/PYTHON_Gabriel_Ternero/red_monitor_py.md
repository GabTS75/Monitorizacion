...
red_monitor.py
│
├── Importaciones                    → Incluye 'socket' para hostname e IPs ← Novedad
├── Configuración                    → Umbrales de errores/drops + filtros de vista
│
├── Funciones auxiliares
│   ├── limpiar_pantalla()
│   ├── bytes_a_legible()
│   ├── barra_actividad()            → Barra relativa a 1 Gbps ← Nuevo
│   ├── color_velocidad()            → Colorea MB/s según nivel de tráfico ← Nuevo
│   ├── obtener_velocidades_red()    → Doble medición igual que en disco ← Nuevo
│   ├── obtener_ips()                → IPs IPv4 e IPv6 por interfaz ← Nuevo
│   └── obtener_estado_interfaces()  → UP/DOWN, velocidad de enlace, MTU ← Nuevo
│
├── mostrar_red()
│   ├── Sección 1 — Por interfaz     → velocidad, IPs, acumulado, errores/drops
│   ├── Sección 2 — Total global     → suma de todas las interfaces
│   ├── Sección 3 — Conexiones       → resumen agrupado por estado TCP
│   └── Sección 4 — Diagnóstico      → alertas de errores y drops
│
├── iniciar_monitor()
└── if __name__ == ...
```
...
1. **La librería** `socket` — La usamos para dos cosas: obtener el hostname del equipo y para identificar si una dirección es IPv4 (`AF_INET`) o IPv6 (`AF_INET6`). Son las "familias" de direcciones de red.
2. **Conexiones agrupadas por estado** — En lugar de listar cientos de conexiones individuales, las agrupamos por estado. Los estados más importantes que verán los alumnos son estos:

    | Estado | Significado |
    | --- | --- |
    | `ESTABLISHED` | Conexión activa y comunicándose |
    | `LISTEN` | Puerto abierto esperando conexiones entrantes |
    | `TIME_WAIT` | Conexión cerrándose (espera normal del protocolo TCP) |
    | `CLOSE_WAIT` | El otro extremo cerró, esperamos nuestro cierre |

3. `AccessDenied` **en conexiones** — En Linux sin `sudo`, psutil no puede ver las conexiones de otros usuarios. El script lo captura y da una instrucción clara en lugar de un error feo.

> 💡 **Analogía de paquetes y drops:** Imagina una autopista con un peaje. Los coches son los paquetes de datos. Los "errores" son coches que llegan con daños (el mensaje llegó corrupto). Los "drops" son coches a los que el peaje les dice "no caben más, daos la vuelta" porque hay demasiada cola. Una autopista sana tiene cero accidentes y cero coches rechazados.
...
#  🗑️  Drops  → paquetes descartados porque el sistema estaba demasiado ocupado
#
...
![image.png](red_monitor_py/image.png)

---

### En Linux

![image1.png](red_monitor_py/image1.png)
