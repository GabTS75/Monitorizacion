...
¡Ojo!, la GPU es **el componente más difícil de monitorizar** de forma universal, porque cada fabricante tiene su propia herramienta y `psutil` directamente **no lee GPUs**. El script lo gestiona con elegancia, observemos!.
...
gpu_monitor.py
│
├── Importaciones                    → Incluye 'subprocess' y 'shutil' ← Novedades
├── Configuración
│
├── Funciones auxiliares
│   ├── limpiar_pantalla(), bytes_a_legible(), barra_progreso()
│   ├── color_por_nivel()
│   └── color_temperatura()
│
├── Bloque NVIDIA
│   ├── nvidia_disponible()          → Detecta si nvidia-smi está en el PATH
│   ├── obtener_datos_nvidia()       → Ejecuta nvidia-smi y parsea el CSV
│   └── obtener_procesos_nvidia()    → Lista procesos usando la GPU
│
├── Bloque AMD (Linux)
│   └── obtener_datos_amd_linux()    → Lee /sys/class/drm/ directamente
│
├── mostrar_gpu()
│   ├── CASO 1 — NVIDIA              → Panel completo con todos los datos
│   ├── CASO 2 — Linux sin NVIDIA    → Intenta AMD via /sys, o explica la situación
│   └── CASO 3 — Windows sin NVIDIA  → Sugiere alternativas para AMD/Intel
│
├── iniciar_monitor()
└── if __name__ == ...
```

### Las dos grandes novedades de este script

1. `subprocess` **— llamar a programas externos desde Python**
Este es un concepto muy poderoso. Python puede ejecutar cualquier comando de la terminal y capturar lo que ese comando imprime en pantalla. Es como si Python abriera una terminal, escribiera `nvidia-smi`, leyera la respuesta y la procesara.

    ```python
    Python  ──►  subprocess.run("nvidia-smi ...")
                        │
                        ▼
                 nvidia-smi se ejecuta
                        │
                        ▼
    Python  ◄──  "RTX 3080, 45°C, 23%, 4GB/10GB..."
                        │
                        ▼
                 parseamos y pintamos
    ```

2. `shutil.which()` **— ¿existe este comando en el sistema?.** Antes de llamar a `nvidia-smi`, comprobamos si existe. Es exactamente lo mismo que cuando en Linux escribes `which nvidia-smi` en la terminal para ver si está instalado. Si no existe, Python no lanza ningún error: simplemente cambia de plan.

> 💡 **Analogía de los 3 casos:** El script es como un médico que llega a una consulta sin saber qué paciente va a encontrar. Primero comprueba si tiene la herramienta específica para **NVIDIA** (estetoscopio especializado). Si no, intenta los medios básicos disponibles en Linux (/sys). Y si nada funciona, te da las indicaciones para ir al especialista adecuado. Nunca se rinde ni da un error incomprensible.

### ⚠️ Lo que se verá, según la situación

| Entorno | Resultado esperado |
| --- | --- |
| PC físico con GPU NVIDIA | Panel completo con todos los datos |
| PC físico con GPU AMD en Linux | Temperatura, uso y VRAM básicos |
| VirtualBox / VMware / WSL | Mensaje informativo — sin datos reales |
| Windows sin drivers NVIDIA | Sugerencia de herramientas alternativas |
...
![image.png](gpu_monitor_py/image.png)
...
![image1.png](gpu_monitor_py/image1.png)
