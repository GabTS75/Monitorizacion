...
Empecemos con el primero, pero antes haremos un “mapeo” de lo que hace cada parte este script, para que se entienda mejor.
...
cpu_monitor.py
│
├── Importaciones            → Las "cajas de herramientas" que necesitamos
├── Configuración            → Parámetros fáciles de cambiar (intervalo, umbral...)
│
├── Funciones auxiliares
│   ├── limpiar_pantalla()   → Borra la terminal para que se vea limpio
│   ├── barra_progreso()     → Convierte un % en una barra visual █████░░░░
│   └── color_por_uso()      → Verde / Amarillo / Rojo según el nivel de uso
│
├── mostrar_cpu()            → El "corazón": obtiene y pinta todos los datos
├── iniciar_monitor()        → El bucle infinito que llama a mostrar_cpu() cada Xs
│
└── if __name__ == ...       → La puerta de entrada: solo arranca si lo ejecutas tú
```

> 💡 **Analogía del bucle** `while True`**:** Imagina que contratas a un vigilante de seguridad. Le das instrucciones de: *"mira las cámaras, apunta lo que ves, espera 2 segundos, y repite"*. El vigilante no para nunca... hasta que tú le dices "para" (`Ctrl+C`). Eso es exactamente el bucle `while True` con `KeyboardInterrupt`.
...
![image.png](cpu_monitor_py/image.png)

---

### En Linux

![image1.png](cpu_monitor_py/image1.png)
