# sistema_info.py (info estática)

Este script es diferente a todos los anteriores, no es un monitor en bucle sino una **fotografía completa del sistema** que se ejecuta una vez y muestra toda la información estática del hardware. Es perfecto como inicio antes de monitorizar.
...
sistema_info.py
│
├── Importaciones                → Incluye 'socket' y 'time' además de los habituales
├── Funciones auxiliares
│   ├── bytes_a_legible()
│   ├── seccion()                → Cabecera uniforme para cada bloque ← Nueva
│   └── dato()                   → Línea etiqueta/valor alineada ← Nueva
│
├── Secciones del informe
│   ├── mostrar_encabezado()     → Fecha, hostname, SO
│   ├── mostrar_so()             → SO completo, hostname, IP, uptime
│   ├── mostrar_cpu()            → Modelo, núcleos, frecuencias, extensiones
│   ├── mostrar_ram()            → Total, swap, tipo DDR (si hay dmidecode)
│   ├── mostrar_almacenamiento() → Particiones con capacidad, uso y libre
│   ├── mostrar_red()            → Interfaces, IPv4, IPv6, MAC, estado
│   ├── mostrar_usuarios()       → Sesiones activas en el sistema
│   └── mostrar_resumen()        → Tabla compacta con los datos clave
│
├── mostrar_sistema_info()       → Llama todas las secciones en orden
└── if __name__ == ...           → Sin bucle while — se ejecuta UNA sola vez
```

### Lo que hace diferente a este script

**La diferencia filosófica fundamental:** todos los scripts anteriores son monitores con `while True`. Este no. Se ejecuta, muestra la información y termina. Es más parecido a un comando del sistema como `lscpu` o `uname -a` que a `top` o `htop`.

`dmidecode` **— el comando que lo sabe todo del hardware**
En Linux, `dmidecode` puede leer la tabla DMI/SMBIOS de la placa base, donde el fabricante escribe información del hardware durante el ensamblaje de fábrica: tipo de RAM, slots disponibles, número de serie, versión de BIOS... El script lo intenta, pero necesita `sudo`. Si no está disponible, simplemente lo omite sin error.

**La dirección MAC:** Aparece en la sección de red con `psutil.AF_LINK`. Es la dirección física grabada en el chip de la tarjeta de red, única en el mundo. A diferencia de la IP que puede cambiar, la MAC es permanente (aunque puede falsificarse por software, algo que los alumnos de redes seguro que ya conocen 😉).

> 💡 **Analogía del informe estático vs monitor dinámico:** La diferencia entre `sistema_info.py` y el resto de scripts es la misma que hay entre la **ficha técnica de un coche** (cilindrada, potencia, peso — datos fijos) y el **cuadro de instrumentos mientras conduces** (velocidad, rpm, temperatura del motor — datos en tiempo real). Necesitas ambas cosas: la ficha para saber con qué equipo trabajas, y los monitores para saber cómo está funcionando ahora mismo.
...
![image.png](sistema_info_py/image.png)
![image1.png](sistema_info_py/image1.png)

---

### En Linux

![image2.png](sistema_info_py/image2.png)
![image3.png](sistema_info_py/image3.png)
![image4.png](sistema_info_py/image4.png)
