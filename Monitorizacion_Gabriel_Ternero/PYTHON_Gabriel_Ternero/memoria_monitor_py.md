...
Lo que hace cada parte del script, así se entiende mucho mejor:

```python
memoria_monitor.py
│
├── Importaciones
├── Configuración              → Umbrales separados: RAM (85%) y Swap (50%)
│
├── Funciones auxiliares
│   ├── limpiar_pantalla()
│   ├── bytes_a_legible()      → 8.589.934.592 bytes → "8.00 GB"  ← ¡Novedad!
│   ├── barra_progreso()
│   ├── color_por_uso()        → Ahora acepta umbral como parámetro ← Mejorada
│   └── linea_detalle()        → Formato alineado de etiqueta + valor ← ¡Novedad!
│
├── mostrar_memoria()
│   ├── Sección 1 — RAM        → Total, usada, libre, disponible, caché, buffers
│   ├── Sección 2 — Swap       → Total, usada, libre, actividad entrada/salida
│   └── Sección 3 — Diagnóstico rápido global
│
├── iniciar_monitor()          → Bucle while True
└── if __name__ == ...
```

### Novedades

Dos cosas nuevas que aparecen aquí y no estaban antes, y que merece la pena tener muy en cuenta:

**1.** `bytes_a_legible()` — La memoria se mide en bytes internamente, pero los humanos pensamos en GB. Esta función hace la traducción automáticamente eligiendo la unidad adecuada según el tamaño.

**2.** `hasattr()` — Antes de usar un atributo que puede no existir en Windows, preguntamos si existe. Es como llamar a una puerta antes de entrar: evita errores inesperados.

> 💡 **Analogía del Swap:** Si la **RAM** es tu mesa de trabajo y el disco duro es el archivador del otro lado de la oficina, el **Swap** es levantarte a buscar carpetas cada vez que tu mesa se llena. Puedes seguir trabajando, pero mucho más despacio. Si ves que el **Swap** está muy alto, es la señal de que necesitas una mesa más grande, es decir, **necesitas más RAM**.
...
![image.png](memoria_monitor_py/image.png)

---

### En Linux

![image1.png](memoria_monitor_py/image1.png)
