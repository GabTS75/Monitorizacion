...
Las temperaturas son especialmente importantes porque sabemos que un equipo con calor excesivo puede dañarse físicamente. Hay que tener en cuenta que este script es el más **dependiente** del sistema operativo de todos.

## Mapeo de este script

```python
temperatura_monitor.py
│
├── Importaciones              → Incluye 'platform' para detectar el SO ← Novedad
├── Configuración              → Umbrales por tipo de componente (dict UMBRALES)
│
├── Funciones auxiliares
│   ├── limpiar_pantalla()
│   ├── barra_temperatura()    → Barra en °C, no en % ← Nuevo
│   ├── formato_temperatura()  → Color + símbolo + barra integrados ← Nuevo
│   └── identificar_sensor()   → Clasifica sensores por palabras clave ← Nuevo
│
├── mostrar_temperaturas()
│   ├── Bloque Windows         → Detecta que no hay soporte y sugiere alternativas
│   ├── Sin sensores           → Guía de activación de módulos del kernel
│   ├── Sección 1              → Todos los sensores agrupados por chip
│   ├── Sección 2              → Alertas activas en tiempo real
│   ├── Sección 3              → Top 5 temperaturas más altas
│   └── Sección 4              → Diagnóstico global de refrigeración
│
├── iniciar_monitor()
└── if __name__ == ...
```
...
1. **Detección del sistema operativo con** `platform`
Antes de intentar leer sensores, el script pregunta en qué sistema está. Si es Windows, no intenta nada y muestra herramientas alternativas. Es mejor dar una explicación clara que un error incomprensible.
2. **Umbrales por componente con un diccionario**
En lugar de una variable suelta como antes, usamos un diccionario `UMBRALES` donde cada tipo de componente tiene su propio par `(advertencia, crítico)`. Es más limpio y fácil de ampliar.
3. `identificar_sensor()`**, el "traductor"**
Los nombres que devuelve `psutil` son técnicos y varían según el fabricante: `coretemp`, `k10temp`, `nct6775`... Esta función los "traduce" a categorías legibles buscando palabras clave.

> 💡 **Analogía del throttling:** Imagina que eres un corredor de maratón. Si llevas demasiado tiempo corriendo y tu cuerpo se sobrecalienta, automáticamente reduces el ritmo aunque tú quieras seguir igual. Eso hace exactamente la CPU cuando su temperatura sube demasiado: se "autofrena" para no quemarse. A ese freno involuntario se le llama **throttling térmico**.

### 🐧 En Linux (si no aparecen sensores)

```bash
# Instalar lm-sensors
sudo apt install lm-sensors

# Detectar sensores disponibles (responde "yes" a todo)
sudo sensors-detect

# Cargar el módulo específico según tu CPU
...
⚠️  Lectura de temperaturas no disponible en Windows

En Windows `psutil` **NO** soporta lectura de temperaturas en Windows, en ese caso el script lo detecta y avisa al usuario, sugiriendo herramientas alternativas (ej. HWiNFO, Core Temp, etc.).

---

### En Linux

![image.png](temperatura_monitor_py/image.png)

¡Completamente normal!

Las máquinas virtuales no tienen acceso a los sensores físicos del host, es una limitación esperada. El script funciona correctamente, simplemente no hay sensores que leer.

En un equipo físico real, seguro funcionará perfectamente.
