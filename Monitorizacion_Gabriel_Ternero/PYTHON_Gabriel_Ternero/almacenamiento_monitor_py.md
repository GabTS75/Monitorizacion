...
│
├── Importaciones
├── Configuración               → Dos umbrales de disco: alerta (85%) y crítico (95%)
│
├── Funciones auxiliares
│   ├── limpiar_pantalla()
│   ├── bytes_a_legible()
│   ├── barra_progreso()
│   ├── color_disco()          → Ahora tiene 4 niveles: verde/amarillo/rojo/rojo parpadeante ← Novedad
│   └── obtener_velocidad_io() → Doble medición para calcular MB/s en tiempo real ← ¡Clave!
│
├── mostrar_almacenamiento()
│   ├── Sección 1 — Espacio por partición      → tabla con total/usado/libre/uso%
│   ├── Sección 2 — Velocidad I/O tiempo real  → lectura y escritura en MB/s
│   ├── Sección 3 — Estadísticas acumuladas    → operaciones totales desde arranque
│   └── Sección 4 — Dispositivos físicos       → tipo, punto de montaje, modo
│
├── iniciar_monitor()
└── if __name__ == ...
```

### La clave: medir velocidad con dos lecturas

Este script introduce un concepto nuevo que no habíamos necesitado antes: **medir algo que cambia con el tiempo**. El truco está en `obtener_velocidad_io()`, aquí la explicación:

```python
  Lectura 1 ──── espera 1 seg. ──── Lectura 2
      ↑                                 ↑
bytes_leídos = 500 MB             bytes_leídos = 620 MB
                                   
Velocidad = (620 - 500) / 1 segundo = 120 MB/s  ✅
```

> 💡 **Analogía:** Para que se entienda mejor, esto es exactamente como medir la velocidad de un coche en una carretera sin radar. Si a las 12:00:00 está en el kilómetro 50, y a las 12:00:01 está en el kilómetro 50,033... ha viajado 33 metros en 1 segundo, es decir, va a 120 km/h.
>
> La misma lógica, aplicada a bytes. 👈

### ⚠️ Una cosa a tener en cuenta en Windows

En Windows, algunas particiones del sistema pueden devolver `PermissionError` al leer su uso. El script lo gestiona silenciosamente saltándose esas particiones, así que no verás errores en pantalla.
...
![image.png](almacenamiento_monitor_py/image.png)

---

### En Linux

![image1.png](almacenamiento_monitor_py/image1.png)
